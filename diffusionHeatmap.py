...
df = pd.read_csv('dump.COMNH3_1216A.lammpstrj', skiprows = 9, names=['id', 'mol', 'type', 'x', 'y', 'z'], delim_whitespace=True)
sns.set(rc={'figure.figsize':(20,20)})
framework_img = mpimg.imread('1900Frame1.png') # read in still framework file
fig = plt.figure()
...
htmp = sns.kdeplot(df.y, df.z,
                 cmap="Reds", shade=True, shade_lowest= False, n_levels = 25, gridsize = 100, bw = 2.75, ax = ax)#, cut=0) # bw=.15,
