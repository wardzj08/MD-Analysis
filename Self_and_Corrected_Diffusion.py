...
input_file = r'./data/dump.TST2' # file to read trajectory from
snapshots_to_read = 1000# Number of timesteps to read from traj file
...
def distance(x1, x0):
    x0 = x0 - math.floor(x0/BoxSize) * BoxSize
    dx = x1 - x0
    dx = dx - int(round(dx/BoxSize)) * BoxSize
    return dx
...
for snp in range(snapshots_to_read):
 ...
 # Calculate x-x0, y-y0, z-z0 for each mol COM
        ...
        for mol in range(num_atoms):
            ...
            #print(df_mol.iloc[[mol]])
            #print(df_mol_init.iloc[[mol]])
            xdiff = distance(df.iloc[[mol]]['x'].values[0] , df_atom_init.iloc[[mol]]['x'].values[0])# df_mol.iloc[[mol]]['x'] - df_mol_init.iloc[[mol]]['x']
            ydiff = distance(df.iloc[[mol]]['y'].values[0] , df_atom_init.iloc[[mol]]['y'].values[0]) #df_mol.iloc[[mol]]['y'] - df_mol_init.iloc[[mol]]['y']
            zdiff = distance(df.iloc[[mol]]['z'].values[0], df_atom_init.iloc[[mol]]['z'].values[0])#df_mol.iloc[[mol]]['z'] - df_mol_init.iloc[[mol]]['z']
