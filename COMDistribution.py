...
def calc_center_of_masses(input_file, mass_dict, snapshots_to_read, num_atoms, atoms_per_adsorbate, MOF_atoms):
    ...
    for snp in range(snapshots_to_read):
        ...
        if snp == 0:
           df = pd.read_csv(input_file, skiprows = 9, nrows = num_atoms, names = ['mol', 'element', 'x', 'y', 'z'], usecols=[1,3,4,5,6], delim_whitespace=True)
        else:
            skips = num_atoms*snp + 9*(snp+1)
            df = pd.read_csv(input_file, skiprows = skips, nrows = num_atoms, error_bad_lines = False, names = ['mol', 'element', 'x', 'y', 'z'], usecols=[1,3,4,5,6], delim_whitespace=True)
        ...
        df_grouped = df_grouped.apply(lambda x: x.sort_values(['element'], ascending = False))
	...
	# print(df)
	    ...
	    # List to hold each atom_mass * atm_position value for an individual molecule
            ...
            for k in range(atoms_per_adsorbate):
                ...
                #print('\n',atm['element'])
                atom_calc = masses[atm['element']] * atm[['x', 'y', 'z']]
